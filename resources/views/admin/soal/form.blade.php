                ...
                <div class="mb-3 row">
                    ...
                    <div class="col-sm-10">
                        <select name="jenis_ujian" id="jenis_ujian" class="form-control" required>
                            ...
                            @foreach ($jenis_ujian_list as $ju)
                                <option value="{{ $ju->nama }}"
                                    {{ old('jenis_ujian', $isEdit ? $data->jenis_ujian : '') == $ju->nama ? 'selected' : '' }}>
                                    {{ $ju->nama }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="mb-3 row">
                    ...
                    <div class="col-sm-10">
                        <select name="jenis_soal" id="jenis_soal" class="form-control" required>
                            ...
                            <option value="pilihan_ganda" {{ old('jenis_soal', $isEdit ? $data->jenis_soal : '') == 'pilihan_ganda' ? 'selected' : '' }}>
                                Pilihan Ganda</option>
                            <option value="pilihan_ganda_kompleks"
                                {{ old('jenis_soal', $isEdit ? $data->jenis_soal : '') == 'pilihan_ganda_kompleks' ? 'selected' : '' }}>Pilihan Ganda
                                Kompleks</option>
                            <option value="menjodohkan" {{ old('jenis_soal', $isEdit ? $data->jenis_soal : '') == 'menjodohkan' ? 'selected' : '' }}>
                                Menjodohkan</option>
                            <option value="uraian_singkat"
                                {{ old('jenis_soal', $isEdit ? $data->jenis_soal : '') == 'uraian_singkat' ? 'selected' : '' }}>Uraian Singkat</option>
                        </select>
                    </div>
                </div>
                ...
                <div id="pilihan-ganda-fields" style="display: none;">
                    @foreach (['a', 'b', 'c', 'd', 'e'] as $opt)
                        <div class="mb-3 row">
                            ...
                            <div class="col-sm-10">
                                <input type="text" name="pilihanganda_{{ $opt }}" class="form-control"
                                    value="{{ old('pilihanganda_' . $opt, $isEdit ? $data->{'pilihanganda_' . $opt} : '') }}">
                            </div>
                        </div>
                    @endforeach
                    <div class="mb-3 row" id="kunci-jawaban-pg">
                        ...
                        <div class="col-sm-10">
                            <select name="kunci_jawaban" id="kunci_jawaban" class="form-control" required>
                                @foreach (['a', 'b', 'c', 'd', 'e'] as $opt)
                                    <option value="{{ $opt }}"
                                        {{ old('kunci_jawaban', $isEdit ? $data->kunci_jawaban : '') == $opt ? 'selected' : '' }}>{{ strtoupper($opt) }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="col-sm-10">
                            @foreach (['a', 'b', 'c', 'd', 'e'] as $opt)
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="checkbox" name="kunci_jawaban[]" id="kunci_jawaban_{{ $opt }}"
                                        value="{{ $opt }}" {{ in_array($opt, $kunciJawabanKompleks) ? 'checked' : '' }}>
